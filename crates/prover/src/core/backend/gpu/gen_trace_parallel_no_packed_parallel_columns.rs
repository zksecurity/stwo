...
const THREADS_PER_WORKGROUP: u32 = 1 << 8;
...
pub async fn gen_trace_parallel_no_packed_parallel_columns(
    log_n_rows: u32,
) -> (Vec<BaseColumn>, LookupData) {
    ...
    // Load shader
    let shader_source = include_str!("gen_trace_parallel_no_packed_parallel_columns.wgsl");
    ...
    if let Ok(Ok(())) = receiver.recv_async().await {
        ...
        //     println!("GPU Trace[{}].data[1]: {:?}", i, c.data[1]);
        // }
